...
* **Authentication:** Secure, HttpOnly, SameSite Cookie-Based Authentication to ensure seamless, long-lived desktop sessions without client-side token refresh overhead. **MVP scope:** a single seeded demo user is auto-signed-in via cookie auth, with the data model and pipeline structured so full ASP.NET Core Identity (registration/login) can drop in later without schema changes.
